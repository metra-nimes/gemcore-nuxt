...
<script>
  export default {
    ...
    data() {
      return {
        form: {
          body: this.$store.getters.getNoteBody,
          title: this.$store.getters.getNoteTitle,
        },
        sending: false,
        responseErr:null,
      }
      ...
      async onSend() {
        ...
        try {
          await this.$store.dispatch('editNote',  {
            ...
            id : this.$store.getters.noteId,
          }).then(() => {
            this.sending = false
            this.$root.$emit('bv::hide::modal','modal')
            this.clean()
            this.$toast.success('Note updated',{duration:2000,position:'bottom-right'})
          })
        } catch (e) {
          this.sending = false
